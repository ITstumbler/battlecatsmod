            ...
            with open("%s\\%s" % (newfoldername, filename),"rb") as file:
                ...
                try:
                  vars()[filename]
                    ...
                    with open("%s\\%s" % (newfoldername, filename),"rb") as filetowrite:
                        whattowrite = filetowrite.read()
                        filedestination_path = os.path.join(my_path, "0pasted\\%s" % (filename))
                        filedestination = open(filedestination_path,"wb")
                        filedestination.write(whattowrite)
                else:
